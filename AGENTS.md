...
- `--min-nuclei-diameter-um`: Smallest nucleus diameter in micrometers to keep (default: 5.0)
- `--max-nuclei-diameter-um`: Largest nucleus diameter in micrometers to keep (default: 20.0)
...
4. **Size filtering formula**: The min/max voxel count bounds use an approximate spherical volume formula with arbitrary scaling factors (`/10` and `*10`). Adjust `--min-nuclei-diameter-um` and `--max-nuclei-diameter-um` if segmentation misses expected nuclei. Physical size is converted to voxel counts using the per-file pixel spacing read via BioImage (`physical_pixel_sizes` + `dimension_properties`)
...
**Changing nucleus size**: Adjust `--min-nuclei-diameter-um` and/or `--max-nuclei-diameter-um`
